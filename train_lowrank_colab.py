"""
Low-rank weight expansion experiment for parameter golf.

Idea: store small rank-R factor matrices (A, B) per layer.
At inference, expand to full weight matrix: W = A @ B
This gives ~32x compression vs storing full weights.

Store:   A (out×R) + B (R×in)  = (out+in)*R params
Expand:  W (out×in)             = out*in params
Ratio:   out*in / ((out+in)*R)  ≈ dim/(2*R) for square matrices

At R=16, dim=1024: ratio = 1024/32 = 32x compression
At R=32, dim=1024: ratio = 1024/64 = 16x compression

This means we can store a much larger "effective" model in 16MB.
The expanded weights are full precision — no ternary precision loss.

Compare against:
    Baseline int8 (dim=512, ~17M params, ~11.5MB):
        ITERATIONS=5000 VAL_LOSS_EVERY=500 TRAIN_BATCH_TOKENS=65536 \\
        torchrun --standalone --nproc_per_node=1 train_gpt.py

    This script (dim=1024, rank=16, ~4M stored params, expands to ~67M):
        ITERATIONS=5000 VAL_LOSS_EVERY=500 TRAIN_BATCH_TOKENS=65536 \\
        python train_lowrank_colab.py
"""
...
class Hyperparameters:
    ...
    # Model dimensions — large since low-rank compression is aggressive
    ...
    # Low-rank rank. Higher = more expressive, larger stored size.
    # R=16:  ~32x compression on square matrices
    # R=32:  ~16x compression
    # R=64:  ~8x compression
...
class LowRankLinear(nn.Module):
    """
    Linear layer stored as two small factor matrices: W = A @ B
    Stored params:  (out_features * rank) + (rank * in_features)
    Effective params: out_features * in_features
    Compression ratio: out*in / ((out+in)*rank) ≈ dim/(2*rank) for square

    At inference, W is expanded once and used as a normal weight matrix.
    Full precision throughout — no quantization error.
    """
    def __init__(self, in_features: int, out_features: int, rank: int):
        ...
        # Init: A normal, B normal, scaled so A@B has same variance as kaiming
        std = 1.0 / math.sqrt(in_features)
        self.A = nn.Parameter(torch.randn(out_features, rank) * std)
        self.B = nn.Parameter(torch.randn(rank, in_features) * 1.0)

    def expand(self) -> Tensor:
        """Expand to full weight matrix. Called at inference or can be cached."""
        return self.A @ self.B

    def forward(self, x: Tensor) -> Tensor:
        # Expand on every forward pass during training
        # (could cache at eval time for speed)
        w = self.expand()
    ...
    def compression_ratio(self) -> float:
        return self.effective_params() / self.stored_params()
...
class CausalSelfAttention(nn.Module):
    def __init__(self, dim: int, num_heads: int, num_kv_heads: int, rope_base: float, qk_gain_init: float, rank: int):
        super().__init__()
        ...
        self.c_q = LowRankLinear(dim, dim, rank)
        self.c_k = LowRankLinear(dim, kv_dim, rank)
        self.c_v = LowRankLinear(dim, kv_dim, rank)
        self.proj = LowRankLinear(dim, dim, rank)
...
class MLP(nn.Module):
    def __init__(self, dim: int, mlp_mult: int, rank: int):
        super().__init__()
        self.fc = LowRankLinear(dim, dim * mlp_mult, rank)
        self.proj = LowRankLinear(dim * mlp_mult, dim, rank)
...
class Block(nn.Module):
    def __init__(self, dim: int, num_heads: int, num_kv_heads: int, mlp_mult: int, rope_base: float, qk_gain_init: float, rank: int):
        super().__init__()
    ...
    def __init__(self, args: Hyperparameters):
        ...
        self.blocks = nn.ModuleList([
            Block(args.model_dim, args.num_heads, args.num_kv_heads, args.mlp_mult,
                  args.rope_base, args.qk_gain_init, args.rank)
            for _ in range(args.num_layers)
        ])
        self.final_norm = RMSNorm()
        nn.init.normal_(self.tok_emb.weight, mean=0.0, std=args.tied_embed_init_std)
        # Zero-init output projections (proj.A or proj.B — zero A so A@B=0)
        for b in self.blocks:
            nn.init.zeros_(b.attn.proj.A)
            nn.init.zeros_(b.mlp.proj.A)
...
# ==============================================================================
# COMPRESSION — store A, B factors as int8, expand at load time
# ==============================================================================

def quantize_tensor_int8(arr: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    """Per-row int8 quantization for 2D, per-tensor for 1D."""
    f32 = arr.astype(np.float32)
    if f32.ndim == 2:
        clip_abs = np.quantile(np.abs(f32), 0.9999984, axis=1)
        ...
        return q, scale
    clip_abs = float(np.quantile(np.abs(f32), 0.9999984))
...
def dequantize_tensor_int8(q: np.ndarray, scale: np.ndarray) -> np.ndarray:
    if scale.ndim > 0:
        return q.astype(np.float32) * scale.astype(np.float32).reshape(q.shape[0], *([1] * (q.ndim - 1)))
    return q.astype(np.float32) * float(scale)


def compress_state_dict(state_dict: dict[str, Tensor]) -> tuple[dict, dict]:
    """
    Compress the low-rank factors (A, B) as int8.
    Everything else (embeddings, scales) stored as fp16.
    At load time, dequantize A and B, then expand W = A @ B.
    """
    quantized, scales, passthrough = {}, {}, {}
    stats = {"stored_params": 0, "effective_params": 0, "stored_bytes": 0}

    for name, t in state_dict.items():
        arr = t.detach().cpu().float().numpy()
        stats["effective_params"] += arr.size

        # Quantize all 2D float tensors (the A, B factors)
        if arr.ndim == 2 and arr.size > 1024:
            q, s = quantize_tensor_int8(arr)
            quantized[name] = q
            scales[name] = s
            stats["stored_params"] += arr.size
            stats["stored_bytes"] += q.nbytes + s.nbytes
        else:
            pt = arr.astype(np.float16)
            passthrough[name] = pt
            stats["stored_params"] += arr.size
            stats["stored_bytes"] += pt.nbytes

    obj = {"quantized": quantized, "scales": scales, "passthrough": passthrough}
    return obj, stats


def decompress_state_dict(obj: dict) -> dict[str, Tensor]:
    out = {}
    for name, q in obj["quantized"].items():
        arr = dequantize_tensor_int8(q, obj["scales"][name])
...
def main():
    ...
    logfile = f"logs/{args.run_id}_lowrank.txt"
    print(logfile)
    ...
    # Keep low-rank factors in float32 for optimizer quality
    ...
    block_params_named = list(model.blocks.named_parameters())
    matrix_params = [p for name, p in block_params_named
                     if p.ndim == 2 and not any(pat in name for pat in CONTROL_PATTERNS)]
    scalar_params = [p for name, p in block_params_named
    ...
    sample_lr = model.blocks[0].attn.c_q
    cr = sample_lr.compression_ratio()
    log(f"stored_params:{n_stored:,} rank:{args.rank} dim:{args.model_dim} layers:{args.num_layers}")
    log(f"compression_ratio:{cr:.1f}x per matrix — stored A+B instead of full W")
    ...
    while True:
        ...
        if last_step or (args.val_loss_every > 0 and step % args.val_loss_every == 0):
            ...
            val_loss, val_bpb = eval_val(args, compiled_model, device, val_tokens,
    ...
    # Compress: save A, B factors as int8, expand at load time
    quant_obj, quant_stats = compress_state_dict(model.state_dict())
    buf = io.BytesIO()
    ...
    with open("final_lowrank_model.ptz", "wb") as f:
        f.write(compressed)

    log(f"stored_bytes:{quant_stats['stored_bytes']:,} compressed_model_bytes:{len(compressed):,} "
        f"code_bytes:{code_bytes:,} total:{len(compressed) + code_bytes:,} "
        f"headroom:{16_000_000 - len(compressed) - code_bytes:,}")

    # Roundtrip: load factors, expand W=A@B, re-evaluate
    with open("final_lowrank_model.ptz", "rb") as f:
        recovered_obj = pickle.loads(zlib.decompress(f.read()))
    recovered_state = decompress_state_dict(recovered_obj)
    model.load_state_dict(recovered_state, strict=True)
    torch.cuda.synchronize()
    q_val_loss, q_val_bpb = eval_val(args, compiled_model, device, val_tokens,
                                      base_bytes_lut, has_leading_space_lut, is_boundary_token_lut)
    log(f"final_lowrank_roundtrip val_loss:{q_val_loss:.4f} val_bpb:{q_val_bpb:.4f}")
    log(f"final_lowrank_roundtrip_exact val_loss:{q_val_loss:.8f} val_bpb:{q_val_bpb:.8f}")
